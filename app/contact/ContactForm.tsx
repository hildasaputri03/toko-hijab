...
export default function ContactForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const waMessage = `Halo HijabIndah,%0A%0ANama: ${encodeURIComponent(name)}%0AEmail: ${encodeURIComponent(email)}%0A%0APesan:%0A${encodeURIComponent(message)}`;

    // Ganti nomor WA di bawah dengan nomor toko kamu (format internasional tanpa +)
    const waNumber = '6282334062295'; 
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <p className="text-center text-xs text-slate-500">
        Klik tombol di atas untuk melanjutkan chat dengan kami di WhatsApp
      </p>
    </form>
  );
}
